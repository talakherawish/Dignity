...
A long single day covering a full homepage rebuild around the new hero video, a real production bug found and fixed by measuring the live DOM rather than guessing, several rounds of client feedback on the result, and an afternoon pass on mobile layout and asset size. Two commits (`test`, this machine's git identity) fall outside this specific conversation's own work — the Research/Research Projects rename with its detail-page restyle, and Clippings' return to the About nav — but every commit this day carries the same `Co-Authored-By: Claude Sonnet 5` trailer, so they're logged here rather than split out.

**Morning (06:52–10:27, ~3½ hrs)**

- **Homepage hero replaced with a full-viewport looping background video.** The short text-only hero (in place since 2026-08-27, explicitly called "temporary... pending a proper homepage design" at the time) is gone, replaced with `/landing.mp4` — autoplaying, muted, looped, `playsInline`, and carrying no `controls` attribute at all, so there's no play/pause/progress bar to suppress in the first place rather than one that's merely hidden. A dark gradient overlay was added for legibility, then removed again the same morning at the client's explicit instruction — the video was uploaded exactly as intended and wasn't meant to be darkened.
- **Section height made consistent across devices** — the actual problem behind "I can't find the standard height of a web page": `100vh` on mobile includes the space behind the browser's address bar, so a flat `100vh` hero loads pre-scrolled by a few dozen pixels. Switched to `dvh` (dynamic viewport height, later `svh` — see the mobile-fixes entry below), and corrected for the sticky header's own height (60px/80px/101px at mobile/sm/lg, plus its 1px accent bar and border) so "header + hero" lands on exactly one screen instead of one screen plus a sliver. A shared `FULL_SCREEN_SECTION` height constant was pulled out once News and a new Posters section needed the exact same treatment (below).
- **News & Announcements reworked to a fixed physical layout per the client's request**: image and text sit on opposite sides depending on language — image right/text left in Arabic, mirrored in English — rather than following whichever side the reading direction would put first. Implemented as a CSS grid with the image first in DOM order; `dir="rtl"`/`dir="ltr"` on the grid mirrors the column order on its own, so one layout serves both languages with no per-language branch.
- **New homepage Posters section** — a horizontally-scrollable row of portrait poster cards (rounded corners, no per-item text) between Pillars and Team, pulling real published posters from Payload. Background color and "up to 4 items" were left to this session's judgment per the client ("you're gonna have to decide for me... but the colors we're already using"); grey was chosen, then corrected once from an initial green.
- **Posters and News capped to one screen each** and given the same `FULL_SCREEN_SECTION` height as the hero, after the client flagged that every section should match the video's height exactly ("header height news is a section and posters is a section") rather than each guessing its own.
- **A real production bug, found by measuring the live site, not by eye.** After the height fix, the client reported (after a hard refresh, ruling out caching) that Posters and News looked oversized, overflowing above and below their grey/white backgrounds, "like an A4 container," and not full width. `getBoundingClientRect()` measurements taken directly against `https://84-13-77-162.sslip.io` found Posters' content wrapper at `1209px` tall against a `332px` section — a flex child had no `min-h-0`, so it refused to shrink below its content's intrinsic size (the default `min-height: auto`), and the section's `justify-content: center` then split the resulting overflow symmetrically, 438px above and 439px below, matching the report exactly. Fixed by adding `min-h-0` through the whole affected chain in both Posters and News, dropping `justify-center`, and restructuring both sections so only the heading text stays in the readable `max-w-7xl` column while the media row runs full-bleed edge-to-edge like the hero video. Verified with before/after measurements on production itself, not assumed from the fix alone.
- **Team section redesigned**: circular profile photos (falling back to an initial letter when no photo is set) with name and title stacked below, three people maximum, and the "View All" link changed from a plain bordered rectangle beside the heading to a centered `rounded-full` pill below the row.
- **Research renamed to Research Projects** in the nav and hub tiles to match the page title it links to; the research detail page switched from its own hand-rolled header to the shared `PageHero` component so its eyebrow/title are centered and colored to match the rest of Activities.
- **Posters/View-All copy cleanup and further sizing fixes**, after a follow-up screenshot: the Posters eyebrow ("Publications") was dropped and its title changed to "A look at our publications" (لمحة عن منشوراتنا); "View All Participants"/"View All Publications" became a plain "View All" everywhere, matching News' link which was already generic; poster card spacing widened slightly; News headline text sized down and the image/text grid split changed from an even 50/50 to 60/40 so the image reads wider; and two eyebrows that had never gotten the Arabic +2pt bump every other eyebrow already carries (the homepage hero and the About page) were brought in line.
- **Clippings restored to the About nav dropdown and hub tile grid** — it had silently fallen out of both on 2026-08-27 when Stickers took its slot (see *Remaining work* #4, now resolved).
- **Pillars section (Research/Dialogue/Partnership) removed from the homepage entirely**, including its now-dead translation keys — not a revert, a scope decision to drop it. The News carousel image was also shrunk relative to the headline list, and Clippings was reworked from publication-style cards into the same justified photo-gallery collage the Photos page uses, with title and date made optional in the admin (the image is now the only required field, since neither shows on the collage anymore).

**Afternoon (13:52–17:42, ~2 hrs across two gaps)**

- **`landing.mp4` compressed from 134MB to ~6MB** — re-encoded to 1080p/30fps H.264 with its (unused, the video is muted) audio track dropped, and specifically to get back under GitHub's 100MB push limit. Supersedes the earlier note in this section about the file being 4.1MB — it had since been swapped for a much larger source upload.
- **Mobile layout fixes, four rounds in quick succession:**
  1. Poster cards capped to a maximum height (`42svh`) so they no longer overflow wider than the screen; the Team/Working-Group row kept as a horizontal 3-up grid at every width instead of stacking to one member per row below `sm:`; `dvh` swapped for `svh` (small viewport height, fixed for the browsing session) on the hero/News/Posters sections so they stop visibly resizing — the hero video "zooming," the News image collapsing — as the phone's address bar shows or hides mid-scroll; footer top bar (copyright/resources/disclaimer) stacked into one column on mobile.
  2. Minutes later: the News image card was found collapsing to a thin sliver on mobile — its content is entirely absolutely-positioned, so it has no in-flow height for the implicit grid row to size itself against, and its own `h-full` resolved against nothing. Fixed by splitting the stack into two explicit `1fr` grid rows. The poster-card cap from round 1 was also found still too close to full screen width at `42svh` and dropped to `30svh`.
  3. Minutes after that: the grid-row fix from round 2 turned out to be Safari-specific-broken — Safari let the content-less image track ignore its `1fr` row height and spill into the row below, hiding the first line of the first headline. Replaced grid rows with flexbox (`flex-1` on each half), which resolves percentage heights on content-less children more consistently than a grid track does. Also fixed the same round: Posters was still forced to a full screen of height on mobile even after its cards were capped short, leaving a block of empty grey space below the row — now only takes the full-screen treatment from `sm:` up, sized to its own content on mobile. Footer Resources links, which were full bordered icon-buttons at every width and wrapped onto their own line on narrow phones, became plain text links below `sm:` (matching Disclaimer/Privacy beside them), keeping the icon+pill treatment for `sm:` and up. The hero video also gained a poster frame image (`landing-poster.jpg`) and an explicit `play()` call on mount, defensively, for browsers that don't always honor the `autoPlay` attribute — noted as not a fix for iOS Low Power Mode, which blocks video autoplay at the OS level regardless of the page; the native play button visible there is WebKit's own UI, not a site bug.
  4. Half an hour later, verified directly against the live site: poster cards were still ignoring the `30svh` cap from round 2, rendering at a full 800×1066px, wider than the screen — `getBoundingClientRect()` confirmed the outer wrapper *was* correctly capped, but the scroller and each `h-full aspect-[3/4]` card inside it were sizing off the section's full `flex-1` space instead. Root cause: `max-height` on a flex-grown item clamps the box itself but isn't reliably treated as a *definite* size for descendants resolving `h-full` against it. Replaced `max-h-[30svh]` with a plain, directly-authored `h-[30svh]` — checked in isolation first (a bare height vs. a percentage-height child) before applying it, to confirm the mechanism rather than guess again.
- **Five media files uploaded through the admin** (two PDFs and three images, Clippings-related), auto-committed by the server the same way every upload is, plus one further screenshot upload just after 17:40.
...
4. ~~Decide where Clippings goes~~ — resolved 2026-09-12: restored to the About nav dropdown and hub tile grid, and reworked into a photo-gallery collage in the same session.
