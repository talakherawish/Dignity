...
- **Mobile layout fixes**: poster cards capped to a maximum height so they no longer overflow wider than the screen; the Team/Working-Group row kept as a horizontal 3-up grid at every width instead of stacking to one member per row below `sm:`; `dvh` swapped for `svh` (small viewport height, fixed for the browsing session) on the hero/News/Posters sections so they stop visibly resizing — the hero video "zooming," the News image collapsing — as the phone's address bar shows or hides mid-scroll; and the footer's top bar (copyright/resources/disclaimer) stacked into one column on mobile instead of three flex-wrap items scattering once one of them wraps to its own line.
